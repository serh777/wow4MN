...
export function useAuthorityAnalysis(identifier: string, options: {
  ...
  // Función para procesar datos según opciones
  const processAuthorityData = async (rawData: any, options: any) => {
    const {
  ...
  const calculateAuthorityGrowth = (evolution: any[]): number => {
  ...
  const calculateParticipationTrend = (evolution: any[]): string => {
  ...
  const getUniqueRoles = (participation: any[]): number => {
    const roles = new Set(participation.map(p => p.role));
    return roles.size;
  };

  // Función para generar insights específicos
  const generateAuthorityInsights = (data: any, options: any) => {
    const insights = [];
    const governance = data.governanceMetrics || {};
    const social = data.socialReputationMetrics || {};
    const technical = data.technicalInfluenceMetrics || {};
